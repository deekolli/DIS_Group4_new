...
namespace DIS_Group10.DataAccess
{
    public static class DbInitializer
    {
        ...
        static string API_KEY = "xeOUK1hsD4oethe5kTM45q37JI8YxaUzfx88B3DN";
        ...
        public static string replaceString(string responseString)
        {
            string responseStringModified = responseString;
            responseStringModified = responseStringModified.Replace("school.name", "fullName");
            responseStringModified = responseStringModified.Replace("school.city", "parkCode");
            responseStringModified = responseStringModified.Replace("school.state", "states");
            responseStringModified = responseStringModified.Replace("latest.programs.cip_4_digit", "activities");
            responseStringModified = responseStringModified.Replace("title", "name");
        ...
        public static void getParks(ApplicationDbContext context)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    responsebody = replaceString(responsebody);
                }

                if (!responsebody.Equals(""))
                {
                    JObject parsedResponse = JObject.Parse(responsebody);
                    JArray parks = (JArray)parsedResponse["results"];
